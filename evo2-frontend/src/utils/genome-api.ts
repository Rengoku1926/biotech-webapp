...
// ---------------------------------------------------------------------------
// Helper: proxy NCBI eUtils calls through our own API route to avoid CORS
// ---------------------------------------------------------------------------
async function ncbiFetch(
  endpoint: "esearch" | "esummary",
  params: Record<string, string>,
): Promise<unknown> {
  const searchParams = new URLSearchParams({ _endpoint: endpoint, ...params });
  const response = await fetch(`/api/ncbi?${searchParams.toString()}`);
  if (!response.ok) {
    throw new Error(`NCBI proxy error: ${response.statusText}`);
  }
...
export async function searchGenes(query: string, genome: string) {
  // clinicaltables.nlm.nih.gov supports CORS — no proxy needed here
...
export async function fetchGeneDetails(geneId: string): Promise<{
  ...
  try {
    const detailData = await ncbiFetch("esummary", {
      db: "gene",
      id: geneId,
      retmode: "json",
    }) as { result?: Record<string, GeneDetailsFromSearch> };
    ...
    if (detailData.result && detailData.result[geneId]) {
      ...
      if (detail.genomicinfo && detail.genomicinfo.length > 0) {
        ...
        if (!info) return { geneDetails: null, geneBounds: null, initialRange: null };
...
export async function fetchGeneSequence(
  ...
  try {
    ...
    // api.genome.ucsc.edu supports CORS — no proxy needed
...
export async function fetchClinvarVariants(
  ...
  // Search — proxied through /api/ncbi
  const searchData = await ncbiFetch("esearch", {
    db: "clinvar",
    term: searchTerm,
    retmode: "json",
    retmax: "20",
  ...
  // Summary — proxied through /api/ncbi
  const summaryData = await ncbiFetch("esummary", {
    db: "clinvar",
    id: variantIds.join(","),
    retmode: "json",
  }) as {
  ...
  if (summaryData.result?.uids) {
    for (const id of summaryData.result.uids) {
      const variant = summaryData.result[id] as {
        title: string;
        obj_type?: string;
