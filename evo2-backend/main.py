...
evo2_image = (
    ...
    .run_commands("pip install --upgrade pip wheel setuptools psutil")
    .run_commands("pip install torch --no-cache-dir")
    .run_commands("pip uninstall -y transformer-engine transformer_engine transformer_engine_torch || true")
    .run_commands("pip install 'transformer_engine[pytorch]==1.13' --no-build-isolation --no-cache-dir")
    .run_commands("pip install flash-attn --no-build-isolation --no-cache-dir")  # ← back, but after torch+TE
    .run_commands("git clone --recurse-submodules https://github.com/ArcInstitute/evo2.git && cd evo2 && pip install -e . --no-cache-dir")
